...
void setMotorDirection(bool forward) {
  ...
  delayMicroseconds(MOTOR_DIRECTION_SETTLE_TIME); // Direction settling time
  motDir = forward ? 2 : 0;
...
void setMotorSpeed(int speed) {
    ...
    stepInterval = map(motSpeed, minSpeed, maxSpeed, MOTOR_STEP_INTERVAL_MAX, MOTOR_STEP_INTERVAL_MIN);
...
// Timing configuration moved to Config.h


//Encoder rotary setup
int encCurrent = initialTemp;
int encIncrement = ENC_INCREMENT;
...
// Screen configuration moved to Config.h
...
// Thermistor configuration moved to Config.h
...
void setup() {
  ...
  // Debounce delay, triggers on HIGH (button release)
  ...
  // Encoder listener: reads encoder state periodically for responsive detection
  mgr.addListener(new EvtTimeListener(ENCODER_READ_INTERVAL, true, (EvtAction)readEncoder));
...
// Motor configuration moved to Config.h
